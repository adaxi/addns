...
my %cmd_args = (
	'config_file' => ['', '-c', '--config', "Configuration File (defaults to /etc/addns.db)"],
	'config_name' => ['', '-x', '--configname', "Configuration Label to Update"],
	'update_host' => ['', '-H', '--host', "Hostname(s) to update, seperated by commas"],
	...
	'system_type' => ['', '-s', '--system', "System type: statdns, dyndns, custom"],
	...
	'interval' => ['', '-x', '--interval', "Update interval for daemon mode", 1]
);
...
# check_files() - make sure we can access the right files before starting

sub check_files {
	open(CACHE, ">>" . $files{'cache'}) or 
		output("Error: could not open cache file \"$files{'cache'}\" for writing",1,2,1); 
	close(CACHE);
}
...
sub send_update 
{
	my(	$server_host,		# Hostname of actual http server
		...
		@hostnames		# Split list of hostnames
	);

	my($updt_host) = $host_v{$curr_host}{'update_host'};
	if($updt_host eq '') { 
		output("send_update: Malformed Hostname \"$updt_host\"... Null Hostname",1,2,1);
	}
	elsif($updt_host !~ m/^[\w,-\.+]+$/) {
		output("send_update: Malformed Hostname \"$updt_host\"... Invalid Characters",1,2,1);
	}
       	else {
		@hostnames = split(/,/, $updt_host);
		# Debug;
		foreach (@hostnames) {
			print "Individual Hostname: \"$_\"\n" if ($debug > 1);
		}
	}
...
# 0 = success. 
# 1 = failure, but resume anyways in case it's involked with --auto. 
# 2 = failed, stop updating this host no matter what.  
# 3 = failed completely, exit (account errors)

	my $c_hostname=0;		# Current Hostname / for multiple updates
	my $msg="";
	my $code=0;

	foreach (@return) {
	# Host inspecific errors (will exit)
		# Account
		 (/!donator/) && return(3, "Insuffient Privilages (need to donate to use feature)");
		 /badauth/ && return(3, "Access Denied: Bad Username/Password Pair");
		# Client
		 /badsys/ && output("Bad system parameter - Check config, then contact dph\@funtaff.com",1,3,1);
		 /badagent/ && output("Client has been blocked.  Contact dph\@funtaff.com ASAP",1,3,1);
		# System
		...
		(/999/ || /911/) && output("Emergency code from server.",1,3,1);
			# (Die)
	# Account specific codes (won't exit until loop is finished)
		# Failure
		 (/notfqdn/) && do {
			 $code = 2;
			 $msg .= ", " if ($c_hostname > 0);
			 $msg .= "Host " . $hostnames[$c_hostname] . " malformed";
			 $c_hostname++;
		 };
		 (/nohost/) && do {
			 $code = 2;
			 $msg .= ", " if ($c_hostname > 0);
			 $msg .= "Host " . $hostnames[$c_hostname] . " does not exist";
			 $c_hostname++;
		 };
		 (/!yours/) && do {
			 $code = 2;
			 $msg .= ", " if ($c_hostname > 0);
			 $msg .= "Host " . $hostnames[$c_hostname] . " is not yours";
			 $c_hostname++;
		 };
		 (/nochg/ || /abuse/) && do {
			 $code = 2;
			 $msg .= ", " if ($c_hostname > 0);
			 $msg .= "Host " . $hostnames[$c_hostname] . " blocked for abuse";
			 $c_hostname++;
		 };

		# Success
		 /good/ && ($offline eq "yes") && do { 
			 $msg .= ", " if ($c_hostname > 0);
			 $msg .= "Host " . $hostnames[$c_hostname] . " set offline";
			 $c_hostname++;
		 };
		 /good/ && do {
			 $msg .= ", " if ($c_hostname > 0);
			 $msg .= "Host " . $hostnames[$c_hostname] . " updated";
			 $c_hostname++;
		 };
	}
	...
	if($out eq "") {
		...
		return(2, "Unknown Failure (try verbose)");
	}
...
{	
	...
	# Check for specification of the config file on the command line
	if($cmd_args{'config_file'}[0]) {
		if( -s $cmd_args{'config_file'}[0]) {
			$files{'config'} = $cmd_args{'config_file'}[0];
		}
	       	else {
			output("Error: config file \"" . $cmd_args{'config_file'}[0] . "\" not found.",1,2,1);
		}
	}
	...
	foreach my $section (keys %config_raw) {
		if($section =~ /^(default|main|daemon|files)$/) { 
		}
		else {
		...
		foreach my $line (@sec_lines) {
			...
			$line =~ m/^\s*([\w\-\._]+)\s*=\s*"?(.+?)"?\s*$/ && do {
				if($section eq "main") {
					...
					# Make sure they can't change the config file on the fly
					if($1 ne "config") {  
						$files{$1} = $2;
					}
				}
	...
	if($cmd_args{'help'}[0]) {
		...
		foreach my $array (keys %cmd_args) {
			unless($cmd_args{$array}[4]==1) {
				printf("%15s%s%3s%s%s", $cmd_args{$array}[2], " | ", 
						$cmd_args{$array}[1], "    ", $cmd_args{$array}[3] . "\n");
